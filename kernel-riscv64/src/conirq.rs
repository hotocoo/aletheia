...
/// (ADR-180). The ring is checked with interrupts MASKED (`sstatus.SIE` clear), and `wfi` still wakes on a pending
/// enabled interrupt with the global enable clear; the IRQ is then taken when they are restored. The old idle ran `wfi` after `pop` had
...
pub fn wait_for_input() {
    let were = mask_irqs();
    // SAFETY: interrupts are masked and this is the only consumer, so no other reference to RING exists.
    let empty = unsafe { (*core::ptr::addr_of!(RING)).is_empty() };
    if empty {
        // SAFETY: a hint with no memory effects; a pending interrupt ends it even with SIE clear.

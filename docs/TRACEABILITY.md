...
| REQ-SMP-001 | SMP / multicore (umbrella: per-CPU scheduling, TLB shootdown, lock audit, x86-64 bring-up) | ADR-021 | kernel/src/smp.rs; kernel-riscv64/src/smp.rs | kernel/src/smp.rs | scripts/vm-e2e.sh | partial |
| REQ-SMP-002 | Secondary-CPU bring-up + cross-core concurrency substrate (PSCI CPU_ON / SBI HSM hart_start, per-CPU TPIDR/tp, shared SpinLock, atomic authorize+execute on real cores, SGI/SSIP IPI) — aarch64 + RISC-V, `-smp 4` VM-gated | ADR-021 | kernel/src/smp.rs; kernel/src/boot.s; kernel-riscv64/src/smp.rs; kernel-riscv64/src/boot.s; kernel-core/src/sync.rs | kernel/src/smp.rs; kernel-riscv64/src/smp.rs; kernel-core/tests/sync.rs | scripts/vm-e2e.sh; scripts/vm-e2e-riscv.sh | delivered |

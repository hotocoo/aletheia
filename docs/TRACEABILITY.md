...
| REQ-SMP-001 | SMP / multicore umbrella — DELIVERED across every slice: secondary bring-up (REQ-SMP-002), per-CPU run queues + work stealing (REQ-SMP-003), cross-core TLB shootdown (REQ-SMP-004), and CPU affinity + cross-core task migration + the lock-hierarchy/atomic-ordering audit (REQ-SMP-005, ADR-028). NUMA-aware placement remains a documented later refinement (ADR-021), not an open gap | ADR-021 | kernel-core/src/smpsched.rs; kernel/src/smp.rs; kernel-riscv64/src/smp.rs; kernel-x86_64/src/smp.rs | kernel-core/tests/smpsched.rs; kernel/src/smp.rs | scripts/e2e-all.sh | delivered |

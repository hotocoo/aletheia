...
| REQ-QUAL-001 | CI-coverage gate (GAPS4 ALET-P0-003): every bootable architecture discovered in the tree must have a boot gate CI executes, both pipelines (GitHub + self-hosted GitLab) must run the same scripts, every VM gate this matrix claims must actually be executed by CI, and STATUS.md must document every script CI runs. Traceability proves evidence EXISTS; this proves it RUNS | ADR-012 | scripts/check-ci-parity.sh | scripts/check-ci-parity.sh | scripts/check-ci-parity.sh | delivered |

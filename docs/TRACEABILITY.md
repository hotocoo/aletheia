...
| REQ-SMP-001 | SMP / multicore umbrella. Delivered slices: secondary bring-up (REQ-SMP-002), per-CPU run queues + work stealing (REQ-SMP-003), cross-core TLB shootdown (REQ-SMP-004). STILL open: preemptive cross-core task migration (a stolen EL0 task resuming on the thief CPU through the TaskContext seam), CPU affinity, and the lock-hierarchy + atomic-ordering audit | ADR-021 | kernel/src/smp.rs; kernel-riscv64/src/smp.rs; kernel-x86_64/src/smp.rs | kernel/src/smp.rs | scripts/vm-e2e.sh | partial |

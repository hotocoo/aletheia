...
| REQ-SMP-001 | SMP / multicore (umbrella: per-CPU-sched parity on RISC-V/x86-64, preemptive cross-core task migration, TLB shootdown, lock audit) | ADR-021 | kernel/src/smp.rs; kernel-riscv64/src/smp.rs; kernel-x86_64/src/smp.rs | kernel/src/smp.rs | scripts/vm-e2e.sh | partial |
...
| REQ-SMP-003 | Per-CPU run queues + cross-core work stealing (ADR-021 Phase 2 policy): exactly-once dispatch, local-first, steal-from-most-loaded, never-two-locks discipline — kernel-core policy host-proved under real threads + VM-gated on real aarch64 cores | ADR-021 | kernel-core/src/smpsched.rs; kernel/src/smp.rs | kernel-core/tests/smpsched.rs; kernel/src/smp.rs | scripts/vm-e2e.sh | delivered |

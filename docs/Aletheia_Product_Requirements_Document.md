...
- **Performance targets:** IPC fast-path round-trip below one bare privilege-boundary (`svc`) crossing on identical hardware (substrate-fair ratio; see §38.10 for the measured evidence and its scope caveats); syscall/trap latency and context-switch cost tracked as regression-gated benchmarks once real scheduling exists.

...
| N2 | **TLS 1.3 client**: certificate validation against a pinned trust root, no downgrade path | **started (ADR-141/142/143)** — the KEY SCHEDULE (`hkdf=9`), the KEY EXCHANGE (`x25519=7`, with RFC 8446's small-order refusal) and the RECORD LAYER (`tlsrecord=9`) are delivered and proved on all three CPUs. The record layer interoperates with OpenSSL in BOTH directions on the host: it opens what OpenSSL sealed and seals byte-identically. Still missing: the handshake state machine, certificate parsing and signature verification, and a trust root. This kernel cannot speak TLS yet |

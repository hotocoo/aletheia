...
| N6 | **Lethe's policy contract adopted natively**: Aletheia's own implementation of the behaviours Lethe's README specifies and proves — HTTPS-first with plaintext refused rather than silently downgraded, tracker hosts refused as third-party requests, a fixed low-entropy user agent, an ephemeral-by-default site-data store. These are policies, not engine code, and they port as invariants | **DELIVERED (ADR-159)** — `kernel-core/src/policy.rs` proves eight invariants against the browser as built: plaintext refused as plaintext (typed or followed) and never rewritten; an operator-filled block list (`block HOST`, eight names) checked BEFORE the trust table and on `back`; the renderer makes no requests (img/script/iframe/stylesheet sources are neither links nor text); a link to another host is third-party by name and dials only a pinned, unblocked host to ITS pin; one fixed user agent (`http::USER_AGENT`), requests differing only in host and path; a `Set-Cookie` kept nowhere, the next request byte-identical; `forget` emptying history, page and links while trust and blocks stay; a fresh navigator holding nothing. `policy=8` on all three CPUs, `console=51`, live in `scripts/https-e2e.sh`. No shipped tracker list: the mechanism is the contract, the names are the operator's |

Track 2 is complete as written (ADR-138..159), and the window path is proved live through the GUI (ADR-160, `scripts/browser-e2e.sh`). N6 is the sense in which Lethe can genuinely be "built in" to Aletheia: not the C++ binary, but

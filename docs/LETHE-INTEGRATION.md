...
| N2 | **TLS 1.3 client**: certificate validation against a pinned trust root, no downgrade path | **started (ADR-141/142)** — the KEY SCHEDULE (`hkdf=9`) and the KEY EXCHANGE (`x25519=7`, with RFC 8446's small-order refusal) are delivered and proved on all three CPUs, both agreeing with independent implementations on the host. Still missing: the record layer, the handshake state machine, certificate parsing and signature verification, and a trust root. This kernel cannot speak TLS yet |

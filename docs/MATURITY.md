...
| Networking | **P** | The driver, ARP and ICMP echo are proved live against a real answering peer on all three targets over both buses (5 invariants per target + host proofs of the wire logic); a reply arriving proves packets well FORMED | No TCP/UDP/DHCP/routing/sockets, no ARP cache, fixed guest address, replies matched synchronously (P2-020 open) |

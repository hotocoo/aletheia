...
| Capability lifetime across a reboot (`capstore`) | **I** | Save/load with the delegation order re-checked on every edge, revocation cascade re-derived, clock monotonicity and id uniqueness enforced, whole-image refusal; 11 in-kernel invariants on all three targets + 19 host proofs (§INV-CAP-LIFE, ADR-048) | The image is checksummed, NOT authenticated — signing waits on key management (ALET-P1-028). Nothing writes it to a disk yet: committing through `persist.rs` is a filesystem ordering decision, tracked in ALET-P1-026. The logical clock is still a caller-supplied constant |
...
| In-kernel risk advisor (frozen integer forest, `mlrisk`) | **P** (the model and its guarantees) / **A** (its use) | The blob is embedded in every image and VERIFIED AT BOOT on all three targets: 20 in-kernel invariants (load, a measured worst-case compare bound, exact margin/verdict parity with the trainer over the whole committed fixture, determinism, out-of-box abstention, nine named refusals) plus 9 host proofs — including that an abstaining model schedules bit-identically to the model-free kernel and that priority is never traded for risk (REQ-ML-001, ADR-056) | ADVISORY by construction and by choice: it reorders tasks of EQUAL priority and nothing else. NOTHING in the kernel derives the 20-feature vector from a live task yet, so on a running machine it currently advises about no one — the wiring is a feature-measurement problem, not a model problem. No in-kernel learning by design; the blob's provenance lives in another repository (`aletheia-ml`); loading it from a capability-scoped, signed file instead of `include_bytes!` is deferred (ADR-056) |
...
| Interactive console: editor + dispatcher | **P** | Fail-closed admission (only printable ASCII enters a line; bounded; refused bytes never echoed), escape sequences PARSED rather than filtered (no byte inside a sequence reaches the line; the parser is never left armed; parameters bounded), a real cursor with mid-line editing, bounded history and Tab completion over the live namespace, refusals named, console writes committed, and a 27-command working set (copy, rename, one-transaction append, create-if-absent that never truncates, prefix search, text and non-text reading, device flush, platform reset) — 40 live invariants per target, 20 host tests incl. a full 256-byte sweep, 18 conformance behaviors, and a three-target scripted-operator gate with a reboot | Kernel-space dispatcher over the kernel's own objects — NOT a user-mode shell over a syscall ABI; the editor writes backspaces and spaces, never cursor-address sequences, and does not know the terminal's width, so a line that wraps redraws wrongly on the wrapped portion; no reverse search, no multi-line editing, ASCII only |

...
| Graphics: virtio-gpu control path (first slice) | **I** | Display-info round trip + the whole 2D resource lifecycle + the device's own error grammar, live on all three targets over both buses (13 invariants per target + wire-format host proofs); local refusals proven silent by a traffic counter | No visible pixels yet (`-display none`), no framebuffer text renderer, no cursor queue, polled completion, backing pages stay DMA-registered for the queue lifetime; compositor/window model/GPU isolation untouched (P2-021 open) |

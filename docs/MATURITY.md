...
| WASM components | **I** | No ambient authority, fuel bounds, attenuated spawn; resource model BEYOND fuel (ADR-065): memory bytes, table elements, stack height/recursion depth, wall-clock deadline at host-call crossings — fail-closed defaults, named kills in the audit event, spawn tree inherits the root envelope | Per-run limits only (per-component install-record limits are future work), single host-call body not clock-interruptible, ABI versioning (P1-022), supply-chain verification (P1-023), dependency resolution as a boundary (P1-024) |

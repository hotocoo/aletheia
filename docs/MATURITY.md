...
| Interactive console: editor + dispatcher | **P** | Fail-closed admission (only printable ASCII enters a line; bounded; refused bytes never echoed), refusals named, console writes committed — 15 live invariants per target, 20 host tests incl. a full 256-byte sweep, 7 conformance behaviors, and a three-target scripted-operator gate with a reboot | Kernel-space dispatcher over the kernel's own objects — NOT a user-mode shell over a syscall ABI; polled; `getc` itself is hardware and unprovable on the host |
| Interactive console: the INPUT path | **P** (ring policy) / **I** (the wire) | The input ring's overflow policy is proved everywhere — 8 live invariants per target, 9 host tests on the SURVIVING contents, 3 conformance behaviors — so a burst can never rewrite a typed command. Real bytes arrive and are acted on: the three-target scripted-operator gate types at a running machine, writes, halts, reboots and reads back | Interrupt-driven on **aarch64 only** (REQ-CON-002 is `partial`): x86-64 and RISC-V still poll. The handler path — GIC ack, FIFO drain, EOI — is hardware, so no host test covers it; framing/parity errors are read past rather than reported, and `run_loop` still spins when the ring is empty |

# ADR-173 — The verification is not slow; the emulator is

**Status:** Accepted (2026-09-12)
**Requirements:** REQ-PERF-001 (advanced)
**Builds on:** ADR-172 (the boot gap is self-verification), ADR-171 (a second operating system),
ADR-056 (the honesty rule).

## Context

ADR-172 attributed Aletheia's ~1047 ms kernel share and found that ~735.6 ms of it — about 70% — is
...
So the first correction is to ADR-172's own wording: a large part of what it called
...
* **The boot comparison is distorted in a third way, and this one favours Linux.** Aletheia's
  ...
  firmware tables and I/O, which TCG handles comparatively well. So the ~735 ms that ADR-172
...
* **That is a reason to be careful, not a reason to claim a win.** One component was measured
  ...
  do. The honest statement is: the absolute boot figures in ADR-169 and ADR-172 are properties of
  the measurement environment as much as of either kernel, and closing that requires hardware.
* **ADR-172's phrasing is corrected, not withdrawn.** Its ~70% figure stands; what changes is that

...
Together with System 2 (DavidAU LFM2.5 on llama.cpp, same workstation): **7/8 right, the same as
System 2 alone, in 37.2 s total against 48.8 s** — System 1 answered `ls` in 175 ms (System 2:
12.8 s) and `rm notes` in 217 ms (960 ms); the rest escalated. No request System 1 answered alone
was wrong. The shipped manifest is therefore `status = "ready"` and is the System-1 default; the

...
| INV-CAP-DELIVERY | Custody anchor crosses the platform boundary | REQ-CAP-008; REQ-BOOT-001 (delivery slice toward Phase 2) | firmware-channel delivery of the 32-byte root through one named door; RootNotProvided / FirmwareAbsent / MalformedRoot refused before any byte is wanted; paired commits witness the vault generation inside the entity-store record and rolled-back custody is refused by name; absence seals the vault while the machine continues | host: kernel-core/tests/bootroot.rs (lying-directory sweeps, wrong sizes, pair-rollback construction, fault-at-every-pair-position); in-kernel: [vault] ALL 14 CUSTODY-DELIVERY INVARIANTS HOLD on all three targets against REAL firmware + persistent medium; third boot without the item asserts the named absence refusal |
...
| INV-VTD | The IOMMU contract meets hardware (x86-64 first rung) | ALET-P1-018 / REQ-DRV-006 (ADR-073) | DMAR/DRHD discovery plus capability-declared register layout, identity domain from OWNED frames with the image audited leaf-free, SRTP/TE handshakes observed, granted function walks clean (bank empty), revoked context denied BY NAME (active record: source-id + CONTEXT_ENTRY_P), restoration returns to silence after a W1C record retirement, enforcement latched to halt layered over the software registry | host: kernel-core/tests/vtd.rs (simulated unit + device-side walker); boot-gate: vm-e2e-x86.sh only (`dmar=14`; VirtualBox skips by named absence — no DMAR declared) |

| INV-VTD-W | Per-device WINDOW domains on VT-d (x86-64 third rung) | ALET-P1-018 / REQ-DRV-006 (ADR-075) | Registry-driven narrowing: every driven function translates ONLY its own granted frames - leaf-set equality audited live against sorted spans, pairwise-disjoint grant sets, ungranted functions read back with NO context entry, SoftIommu mirror agreement under windows, page-granular revocation denied BY NAME (measured reason 6 at exactly the revoked address) while sibling windows keep serving, restore returns silence, enforcement latched layered over the software registry | host: kernel-core/tests/vtd.rs (window domains hold exactly their leaves; page revocation spares sibling windows; context-read reports absence); boot-gate: vm-e2e-x86.sh only (dmar=14) |
| INV-SMMU | The IOMMU contract crosses the ARM fence (aarch64 second rung) | ALET-P1-018 / REQ-DRV-006 (ADR-074) | DT-declared arm,smmu-v3 discovery inside the identity window, iommu-map-bound PCIe requesters (virtio-mmio asserted OUTSIDE), stage-2 identification refused when degenerate, quiet start, identity domain from OWNED frames minus image with counts balancing, live audit zero image violations, grants under DECLARED stream ids, SoftIommu mirror agreement, STRTAB/queue publication with readback, CR0->CR0ACK enablement, latched residency layered over the software registry; device-side walk probes stay open (emulator attachment artifact, abort-canary measured) | host: kernel-core/tests/smmu.rs (simulated unit + SMMUv3-shape walker); boot-gate: vm-e2e.sh aarch64 only (smmu=10) |

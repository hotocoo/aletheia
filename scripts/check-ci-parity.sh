...
#   system1-storm-e2e.sh       needs a serving System-1 sidecar and its 800 MiB model (ADR-188); a
#                              CI runner has neither, and the gate SKIPs by name without them. The
#                              one-option-choice defect it found is pinned by a unit test that CI runs
#                              (ai::dual::tests::a_single_candidate_is_never_asked_and_escalates)
EXEMPT="e2e-all.sh build-all.sh run-interactive.sh vbox-install.sh serial-console.ps1 linux_pipe_bench.sh sbom.py build-example-component.sh lib-markers.sh sync-lethe.sh system1-storm-e2e.sh"

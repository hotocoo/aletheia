...
echo "$OUT" | grep "ALL 38 USER-MODE BOUNDARY INVARIANTS HOLD" >/dev/null || { echo "FAIL: user-mode marker missing"; fail=1; }
...
echo "$OUT" | grep "ALL 59 CONSOLE INVARIANTS HOLD" >/dev/null || { echo "FAIL: console invariants marker missing (REQ-CON-001)"; fail=1; }
...
RISCV_EXPECTED="bench=12 browser=9 cap=14 clock=7 compose=8 compositor=14 conring=10 content=8 policy=8 console=59 dma=9 dns=9 ed25519=8 edid=5 kheap=5 entropy=6 fbcon=6 fs=15 fsstorm=5 lethe=12 lethed=15 linebuf=4 shellstorm=5 gpu=13 hkdf=9 http=8 input=13 iommu=9 keys=12 mlrisk-stress=8 mlrisk=22 mlsched=17 mm=21 net=9 persist=10 pm=14 reclaim=9 selftest=13 sha512=5 smp=22 soak=12 filepanel=13 persona=8 tcp=9 tcpconn=15 tcpnet=3 textgrid=7 tlsclient=8 tlshandshake=12 tlsrecord=9 trust=9 schedstorm=5 wm=14 wmstorm=6 usermode=38 vault=14 vinput=10 virtio=21 x25519=7 x509=9 vm=66"

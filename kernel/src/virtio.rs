...
pub type Input = kernel_core::vinput::VirtioInput<Aarch64Virtio, MmioTransport>;
...
pub fn input_pair() -> Option<Result<(Input, Input), virtioinput::InputError>> {
    let dev_at = |nth: usize| -> Option<Result<Input, virtioinput::InputError>> {
        // SAFETY: the slot addresses are mapped device memory; `new_for` refuses anything that
        // is not a modern input device, and the frames handed to the device are identity-mapped

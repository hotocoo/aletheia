...
fn kmain(memory_map: &MemoryMapOwned) -> ! {
    ...
    // verifier refuses a time of zero because this kernel had no clock; now the CMOS real-time clock
    ...
    kprintln!("");
    kprintln!(
        "--- clock selftests (CMOS real-time clock: plausible, monotonic, verifier-ready) ---"
    );
    let wall_clock = rtc::CmosRtc::new();
    ...
    match gpu_dev.as_mut() {
        ...
        Some(Ok(gpu)) => {
            ...
            match kernel_core::virtiogpu::compose_suite(gpu, |n, passed, name| {
                if passed {
                    kprintln!("  [pass {:>2}] {}", n, name);
                } else {
                    kprintln!("  [FAIL {:>2}] {}", n, name);

...
# The per-device WINDOW rung (ADR-075) requires a 4-level unit; on 3-level-only emulator
# generations the suite SKIPS LOUDLY before programming anything, and this gate holds it to
# the named skip line plus every OTHER family instead.
VT_LINE='ALL 14 VT-D INVARIANTS HOLD'
DMAP='dmar=14'
if grep -q 'window rung SKIPPED' "$LOG"; then
  VT_LINE='window rung SKIPPED: unit offers only a 3-level AGAW'
  DMAP=''
fi
...
if [ "$RC" -eq 33 ] \
   ...
   && grep -q 'ALL 14 COMPOSITION-CONTRACT INVARIANTS HOLD' "$LOG" \
   && grep -q 'ALL 8 REAL-PIXEL COMPOSITION INVARIANTS HOLD' "$LOG" \
   && grep -q 'ALL 13 INPUT-ROUTING INVARIANTS HOLD' "$LOG" \
   && grep -q 'ALL 10 INPUT-HARDWARE INVARIANTS HOLD' "$LOG" \
   && grep -q 'ALL 7 TEXT-GRID INVARIANTS HOLD' "$LOG" \
   && grep -q 'ALL 14 WINDOW-MANAGER INVARIANTS HOLD' "$LOG" \
   ...
   && grep -q 'ALL 5 CONSOLE-STORM INVARIANTS HOLD' "$LOG" \
   && grep -q "$VT_LINE" "$LOG" \
   ...
   && grep -q 'ALL 53 RING-3 BOUNDARY INVARIANTS HOLD' "$LOG" \
...
&& grep -q 'ALL 17 LIVE-ADVISORY INVARIANTS HOLD' "$LOG" \
    ...
    && grep -q 'ALL 9 RECLAIM INVARIANTS HOLD' "$LOG" \
    && grep -q 'storm: pressure entered and cleared, every frame back EXACTLY' "$LOG" \
&& grep -q 'ALL 12 LETHE ADVISOR INVARIANTS HOLD' "$LOG" \
...
&& grep -q 'advised drain is a permutation of the model-free one' "$LOG" \
  ...
  X86_EXPECTED="bench=12 browser=9 cap=14 clock=7 compose=8 compositor=14 conring=10 content=8 policy=8 console=59 dma=9 ${DMAP} dns=9 ed25519=8 edid=5 kheap=5 entropy=6 fbcon=6 fs=15 fsstorm=5 lethe=12 lethed=15 linebuf=4 shellstorm=5 gpu=13 hkdf=9 http=8 input=13 iommu=9 keys=12 mlrisk-stress=8 mlrisk=22 mlsched=17 mm=22 net=9 persist=10 persona=8 filepanel=13 pm=14 ps2=5 reclaim=9 selftest=13 sha512=5 smp=23 soak=12 tcp=9 tcpconn=15 tcpnet=3 textgrid=7 tlsclient=8 tlshandshake=12 tlsrecord=9 trust=9 schedstorm=5 wm=14 wmstorm=6 usermode=53 vault=14 vinput=10 virtio=21 x25519=7 x509=9 vm=72"

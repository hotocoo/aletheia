...
if [ "$RC" -eq 33 ] \
   ...
   && grep -q 'ALL 4 CONSOLE-STORM INVARIANTS HOLD' "$LOG" \
   && grep -q "$VT_LINE" "$LOG" \
...
&& grep -q 'advised drain is a permutation of the model-free one' "$LOG" \
  ...
  X86_EXPECTED="bench=12 browser=8 cap=14 clock=7 compose=8 compositor=14 conring=9 content=8 policy=8 console=51 dma=9 ${DMAP} dns=9 ed25519=8 entropy=6 fbcon=6 fs=15 fsstorm=5 lethe=12 lethed=15 linebuf=4 shellstorm=4 gpu=13 hkdf=9 http=8 input=13 iommu=9 keys=12 mlrisk-stress=8 mlrisk=22 mlsched=17 mm=22 net=9 persist=10 persona=8 filepanel=13 pm=14 ps2=5 reclaim=9 selftest=13 sha512=5 smp=23 soak=12 tcp=9 tcpconn=15 tcpnet=3 textgrid=7 tlsclient=8 tlshandshake=12 tlsrecord=9 trust=9 schedstorm=5 wm=14 wmstorm=6 usermode=40 vault=14 vinput=10 virtio=21 x25519=7 x509=9 vm=72"

...
if [ "$RC" -eq 33 ] \
   ...
   && grep -q 'ALL 12 INPUT-ROUTING INVARIANTS HOLD' "$LOG" \
   && grep -q "$VT_LINE" "$LOG" \
...
&& grep -q 'advised drain is a permutation of the model-free one' "$LOG" \
  ...
  X86_EXPECTED="bench=12 cap=14 compose=8 compositor=14 conring=9 console=42 dma=9 ${DMAP} fbcon=6 fs=15 gpu=13 input=12 iommu=9 keys=12 mlrisk-stress=8 mlrisk=22 mlsched=12 mm=22 net=9 persist=10 pm=14 ps2=5 selftest=13 smp=22 soak=12 usermode=39 vault=14 virtio=21 vm=72"

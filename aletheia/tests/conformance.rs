...
fn dir() -> String {
    std::env::temp_dir()
        .join(format!("aletheia-conf-{}", aletheia::domain::new_id()))
        .to_string_lossy()
        .into_owned()
}

fn owner_token(svc: &mut CoreService) -> String {
    let r = svc.handle(Request::BootstrapOwner {
...
fn uc_create_derive_traverse_through_the_api() {
    ...
    let derived = svc.handle(Request::SubmitIntent {
        ...
        intent: Intent {
            ...
            verb: Verb::Derive {
                ...
                content: "master-v1.wav".into(),
            },
        },
        approve: false,
    });
    assert!(derived.ok, "derive succeeds through the boundary");
    let derived_id = derived.data["result"][0]["derived_id"]
        .as_str()
        .unwrap()
        .to_string();
    ...
    assert!(
        ...
        "world model resolves the derived entity via the API"
    );
    ...
    let denied_audit = svc.handle(Request::QueryAudit {
        caps: vec![],
        limit: 50,
    });
...
fn no_ambient_authority_at_the_boundary() {
    ...
    assert!(denied.data["capability_decision"]
        .as_str()
        .unwrap_or("")
        .contains("DENY"));
}
...
fn destructive_requires_approval_lifecycle_over_the_api() {
    ...
    let attempt = svc.handle(Request::SubmitIntent {
        ...
        intent: Intent {
            subject: "human:owner".into(),
            verb: Verb::Delete { id: id.clone() },
        },
        approve: false,
    });
    assert!(!attempt.ok, "destructive op stops pending approval");
    let approval_id = attempt.data["approval_id"]
        .as_str()
        .expect("pending approval id returned")
        .to_string();
    ...
    let bad_deny = svc.handle(Request::ResolveApproval {
        caps: vec![],
    ...
    assert!(
        !bad_deny.ok,
        "resolving an approval requires authority over the bound action"
    );

    // Human grants it → the bound intent executes (approval confers no authority; caps re-checked).
    let resolved = svc.handle(Request::ResolveApproval {
        caps: vec![owner],
        approval_id,
        granted: true,
    });
...
fn capability_scope_confined_over_the_api() {
    ...
    let e2 = svc.handle(Request::CreateEntity {
        caps: vec![owner.clone()],
        subject: "human:owner".into(),
        etype: EntityType::Document,
        content: "two".into(),
    ...
    let grant = svc.handle(Request::Grant {
        caps: vec![owner],
        subject: "agent:a".into(),
        action: "entity.read".into(),
        scope_entities: vec![id1.clone()],
        approval: false,
    });
    let agent_cap = grant.data["token"].as_str().unwrap().to_string();

    assert!(
        svc.handle(Request::SubmitIntent {
            caps: vec![agent_cap.clone()],
            intent: Intent {
                subject: "agent:a".into(),
                verb: Verb::Read { id: id1 }
            },
            approve: false
        })
        .ok
    ...
    assert!(
        !svc.handle(Request::SubmitIntent {
            ...
            intent: Intent {
                subject: "agent:a".into(),
                verb: Verb::Read { id: id2 }
            },
            approve: false
        })
        .ok,
        "must not read outside granted scope"
    );
}
...
fn same_contract_holds_over_the_endpoint_transport() {
    ...
    let boot = client
        .call(&Request::BootstrapOwner {
            subject: "human:owner".into(),
        })
        .unwrap();
    let owner = boot.data["token"].as_str().unwrap().to_string();
    let created = client
        .call(&Request::CreateEntity {
            caps: vec![owner.clone()],
            subject: "human:owner".into(),
            etype: EntityType::Document,

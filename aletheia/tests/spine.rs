...
fn spine_end_to_end() {
    ...
    assert!(
        !contains(&raw, b"hello world secret"),
        "plaintext leaked into store on disk"
    );
    ...
    let e2 = core2
        .store()
        .get_entity(&e.id)
        .expect("entity survives restart");
    let blob = core2
        .store()
        .get_blob(e2.content_ref.as_ref().unwrap())

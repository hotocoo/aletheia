...
fn dir() -> String {
    std::env::temp_dir()
        .join(format!("aletheia-acc-{}", aletheia::domain::new_id()))
        .to_string_lossy()
        .into_owned()
}
...
impl ModelRuntime for Malformed {
    ...
    fn interpret(&self, _i: &Intent) -> Result<String, ModelError> {
        Ok("{ this is not valid json".into())
    }
}
struct Failing;
...
fn c2_versioning_and_recovery() {
    ...
    let e1 = core
        ...
        .unwrap();
    let e2 = core
        .update_entity(&t, "human:owner", &e1.version_chain, b"v2 content")
        .unwrap();
    ...
    let c1 = core
        .store()
        .get_blob(v1.content_ref.as_ref().unwrap())
        .unwrap();
...
fn c4_relationships_world_model() {
    ...
    assert!(
        results
            .iter()
            .any(|v| v.as_str() == Some(derived_id.as_str())),
        "world model finds derived entity"
    );
}
...
fn c6_capabilities_unforgeable() {
    ...
    let decision = core
        .caps()
        .evaluate("entity.read", &target, std::slice::from_ref(&forged));
    assert!(
        matches!(decision, aletheia::capabilities::Decision::Deny(_)),
        "forged handle must be denied"
    );
...
fn c7_delegation_attenuation() {
    ...
    assert!(eng
        .delegate(
            ...
            "agent:b",
            "entity.read",
            Scope::Entities(vec!["e1".into(), "e2".into()]),
            Constraints::none(),
            "agent:a"
        )
        .is_err());
...
fn c9_destructive_requires_approval() {
    ...
    let tr = core.run_intent(
        &task,
        &t,
        Intent {
            subject: "human:owner".into(),
            verb: Verb::Delete { id: e.id.clone() },
        },
        false,
    );
    assert!(!tr.ok);
    ...
    assert!(
        !core
            ...
            .unwrap()
            .deleted,
        "not deleted without approval"
    );
    // With approval: executes.
    let tr2 = core.handle_intent(
        &t,
        Intent {
            subject: "human:owner".into(),
            verb: Verb::Delete { id: e.id.clone() },
...
fn c11_malformed_output_cannot_execute() {
    ...
    assert!(
        !core
            ...
            .events()
            .iter()
            .any(|ev| ev.etype == "AIActionExecuted"),
        "nothing executed"
    );
...
fn c12_midflight_interpretation_failure_is_safe() {
    ...
    assert!(
        core.store().get_entity(&e.id).is_some(),
        "state intact after model failure"
    );
}
...
fn c16_cancellation_stops_without_side_effects() {
    ...
    assert_eq!(
        core.store().relationships().count(),
        0,
        "no derived_from edge created"
    );
}
...
fn c17_operates_without_model() {
    ...
    let mut core = SysCore::open(
        dir(),
        Box::new(aletheia::intelligence::LocalModelRuntime::new(
            "http://127.0.0.1:1",
        )),
    )
    .unwrap();
...
fn c19_untrusted_content_is_data_not_instruction() {
    ...
    assert_eq!(
        tr.result[0]["content"],
        String::from_utf8_lossy(inj).to_string(),
        "content returned as data"
    );

...
</main><div id="toast" class="toast hidden"></div><div id="palette" class="palette hidden"><div class="palette-card"><input id="paletteInput" placeholder="Jump to a surface or action…"><div id="paletteList"></div></div></div><script>
const tabs=['dashboard','world','capabilities','approvals','audit','trace','setup'];let token=sessionStorage.getItem('aletheia.token')||'';let paletteIndex=0;let toastTimer;let activeTab='dashboard';
...
function show(id){activeTab=id;tabs.forEach(x=>document.getElementById(x).classList.toggle('hidden',x!==id));[...nav.children].forEach((b,i)=>{b.classList.toggle('active',tabs[i]===id);b.setAttribute('aria-current',tabs[i]===id?'page':'false')});if(id==='dashboard')loadStats();if(id==='world')loadWorld();if(id==='capabilities')loadCaps();if(id==='approvals')loadApprovals();if(id==='audit')loadAudit()}
...
function openPalette(){document.getElementById('palette').classList.remove('hidden');document.getElementById('paletteInput').value='';paletteIndex=0;renderPalette();document.getElementById('paletteInput').focus()}
...
async function call(op){let r=await fetch('/api/request',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...op,caps:op.caps||[token]})});let j;try{j=await r.json()}catch(_){throw Error('request returned invalid JSON ('+r.status+')')}if(!r.ok||!j.ok)throw Error(j.error||('request failed ('+r.status+')'));return j.data}
...
function logout(){token='';sessionStorage.removeItem('aletheia.token');setStatus(document.getElementById('setupStatus'),'Session forgotten.');document.getElementById('stats').innerHTML='<div class="card">Bootstrap session first.</div>';show('setup')}
async function bootstrap(){let status=document.getElementById('setupStatus');try{setStatus(status,'Bootstrapping…');let d=await call({op:'BootstrapOwner',subject:document.getElementById('subject').value.trim()||'human:operator',caps:[]});token=d.token;sessionStorage.setItem('aletheia.token',token);setStatus(status,'Session ready for '+d.subject);show('dashboard')}catch(e){setStatus(status,e.message,true)}}

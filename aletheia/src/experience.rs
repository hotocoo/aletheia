...
const tabs=['dashboard','world','capabilities','approvals','audit','trace','setup'];let token=sessionStorage.getItem('aletheia.token')||'';let paletteIndex=0;let toastTimer;
...
function refreshCurrent(){let active=tabs.find(x=>!document.getElementById(x).classList.contains('hidden'));if(active==='dashboard')loadStats();else if(active==='world')loadWorld();else if(active==='capabilities')loadCaps();else if(active==='approvals')loadApprovals();else if(active==='audit')loadAudit();notify('Refreshed '+active)}

...
function logout(){token='';sessionStorage.removeItem('aletheia.token');updateSessionIndicator();setStatus(document.getElementById('setupStatus'),'Session forgotten.');document.getElementById('stats').innerHTML='<div class="card">Bootstrap session first.</div>';show('setup')}
...
updateSessionIndicator();loadStats();
</script></body></html>"##;

...
fn confused_deputy_ambient_authority_is_impossible() {
    ...
    assert!(
        matches!(deny, Decision::Deny(_)),
        "ambient service authority must not leak to a client"
    );
    ...
    assert!(
        matches!(deny_empty, Decision::Deny(_)),
        "no ambient authority: empty offer => deny"
    );
}
...
fn deputy_must_forward_the_clients_capability_not_its_own() {
    ...
    assert!(
        r.ok && r.verified,
        "forwarding the client's own capability authorizes the request"
    );
}
...
fn cannot_launder_authority_from_a_revoked_parent() {
    ...
    assert!(
        laundered.is_err(),
        "a revoked parent cannot mint a valid child"
    );
}
...
fn cannot_launder_a_longer_lived_capability_from_an_expired_one() {
    ...
    assert!(
        longer.is_err(),
        "a child cannot outlive its parent (constraints must not loosen)"
    );
    let never = e.delegate(
        expired,
        "B",
        "entity.derive",
        Scope::All,
    ...
    assert!(
        never.is_err(),
        "cannot delegate a never-expiring child from an expiring parent"
    );
...
fn cannot_launder_a_broader_scope_through_a_transfer() {
    ...
    assert!(
        ...
        "a transfer cannot launder broader authority"
    );
    assert!(
        ch.recv().is_none(),
        "fail-closed: nothing enqueued, no token minted"
    );
}
...
fn revocation_is_immediate_no_stale_authorization_window() {
    ...
    assert!(
        !after.ok && !after.executed,
        "a revoked token authorizes nothing on the next request"
    );
    assert!(matches!(after.authorization, Decision::Deny(_)));
    assert_eq!(
        s.event_count(),
        1,
        "the denied re-attempt records no new event"
    );
}
...
fn targeted_revocation_does_not_disturb_siblings() {
    ...
    let root = e.mint("A", "entity.*", Scope::All, Constraints::none());
    let child1 = e
        .delegate(root, "B", "entity.derive", Scope::All, Constraints::none())
        .unwrap();
    let child2 = e
        .delegate(root, "C", "entity.derive", Scope::All, Constraints::none())
        .unwrap();
    e.revoke(child1);
    assert!(e.is_revoked(child1));
    assert!(
        !e.is_revoked(child2),
        "revoking one delegate leaves its sibling intact"
    );
    assert_eq!(
        e.evaluate("entity.derive", &Target::default(), &[child2]),
        Decision::Allow
    );
    // A brand-new delegate from the live root is valid authority — legitimate, not laundering.
    let child3 = e
        .delegate(root, "D", "entity.derive", Scope::All, Constraints::none())
        .unwrap();
...
fn action_wildcard_does_not_over_match_a_neighbouring_namespace() {
    ...
    assert!(matches!(
        e.evaluate("entityx.delete", &Target::default(), &[cap]),
        Decision::Deny(_)
    ));
    assert!(matches!(
        e.evaluate("other.delete", &Target::default(), &[cap]),
        Decision::Deny(_)
    ));
}

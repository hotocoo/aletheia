...
pub fn run(mut report: impl FnMut(u32, bool, &'static str)) -> Result<u32, (u32, &'static str)> {
    ...
    // 1 — fail closed: no capability => deny.
    {
        ...
        check!(
            ...
            "fail-closed: no capability => deny"
        );
    }
    ...
    // 2 — authorized derive runs the full pipeline: validate->authorize->execute->verify->event.
    {
        ...
        check!(
            r.ok && r.verified && s.event_count() == 1,
            "pipeline: authorized derive verified + event recorded"
        );
    }
    ...
    // 3 — forged/fabricated token is not authority.
    {
        ...
        check!(
            matches!(d, Decision::Deny(_)),
            "unforgeable: fabricated token denied"
        );
    }
    ...
    // 6 — revocation cascades to descendants, immediately.
    {
        ...
        check!(
            matches!(d, Decision::Deny(_)) && e.is_revoked(child),
    ...
    // 7 — malformed (untrusted) plan cannot execute.
    {
        ...
        check!(
            !r.ok && !r.executed && r.validation == "rejected" && s.event_count() == 0,
    ...
    // 9 — scope confinement: capability for entity A does not authorize entity B.
    {
        ...
        check!(
            matches!(d, Decision::Deny(_)),
            "scope confinement: other entity denied"
        );
    }
    ...
    // 10 — secure IPC requires a capability; unauthorized send dropped, authorized delivered.
    {
        ...
        check!(
            dropped && delivered,
            "secure IPC: unauthorized dropped, authorized delivered"
        );
    }

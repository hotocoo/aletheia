...
/// status (55) is proof the code ran rather than merely loaded. The line sits right after the code
/// and is addressed PC-relative. Encodings checked against LLVM's assembler for each CPU.
pub fn hello_code(machine: Machine) -> &'static [u8] {
    match machine {
        // adr x0, line; mov x1, #21; mov x8, #12; svc #0;
        // mov x0,#0; mov x1,#10; 1: add x0,x0,x1; subs x1,x1,#1; b.ne 1b; mov x8,#3; svc #0; b .
        Machine::Aarch64 => &[
            0x80, 0x01, 0x00, 0x10, 0xa1, 0x02, 0x80, 0xd2, 0x88, 0x01, 0x80, 0xd2, 0x01, 0x00,
            0x00, 0xd4, 0x00, 0x00, 0x80, 0xd2, 0x41, 0x01, 0x80, 0xd2, 0x00, 0x00, 0x01, 0x8b,
            0x21, 0x04, 0x00, 0xf1, 0xc1, 0xff, 0xff, 0x54, 0x68, 0x00, 0x80, 0xd2, 0x01, 0x00,
            0x00, 0xd4, 0x00, 0x00, 0x00, 0x14, b'h', b'e', b'l', b'l', b'o', b' ', b'f', b'r',
            b'o', b'm', b' ', b'u', b's', b'e', b'r', b' ', b'm', b'o', b'd', b'e', b'\n',
        ],
        // auipc a0,0; addi a0,a0,52; li a1,21; li a7,12; ecall;
        // li a0,0; li a1,10; 1: add a0,a0,a1; addi a1,a1,-1; bnez a1,1b; li a7,3; ecall; j .
        Machine::Riscv64 => &[
            0x17, 0x05, 0x00, 0x00, 0x13, 0x05, 0x45, 0x03, 0x93, 0x05, 0x50, 0x01, 0x93, 0x08,
            0xc0, 0x00, 0x73, 0x00, 0x00, 0x00, 0x13, 0x05, 0x00, 0x00, 0x93, 0x05, 0xa0, 0x00,
            0x33, 0x05, 0xb5, 0x00, 0x93, 0x85, 0xf5, 0xff, 0xe3, 0x9c, 0x05, 0xfe, 0x93, 0x08,
            0x30, 0x00, 0x73, 0x00, 0x00, 0x00, 0x6f, 0x00, 0x00, 0x00, b'h', b'e', b'l', b'l',
            b'o', b' ', b'f', b'r', b'o', b'm', b' ', b'u', b's', b'e', b'r', b' ', b'm', b'o',
            b'd', b'e', b'\n',
        ],
        // lea rdi,[rip+line]; mov esi,21; mov eax,12; int 0x80;
        // xor edi,edi; mov ecx,10; 2: add rdi,rcx; dec rcx; jnz 2b; mov eax,3; int 0x80; jmp .
        Machine::X86_64 => &[
            0x48, 0x8d, 0x3d, 0x24, 0x00, 0x00, 0x00, 0xbe, 0x15, 0x00, 0x00, 0x00, 0xb8, 0x0c,
            0x00, 0x00, 0x00, 0xcd, 0x80, 0x31, 0xff, 0xb9, 0x0a, 0x00, 0x00, 0x00, 0x48, 0x01,
            0xcf, 0x48, 0xff, 0xc9, 0x75, 0xf8, 0xb8, 0x03, 0x00, 0x00, 0x00, 0xcd, 0x80, 0xeb,
            0xfe, b'h', b'e', b'l', b'l', b'o', b' ', b'f', b'r', b'o', b'm', b' ', b'u', b's',
            b'e', b'r', b' ', b'm', b'o', b'd', b'e', b'\n',
        ],
    }
}

/// A program that calls `SYS_WRITE_CONSOLE(addr, len)` twice and exits with the second call's
/// return (ADR-204): the boot suite's probe of the write path's refusals and bound. `addr` and `len`
/// are two 64-bit words at the end of the code, loaded PC-relative (8-aligned at `code_va + 120`).
pub fn writer_code(machine: Machine, addr: u64, len: u64) -> Vec<u8> {
    let template: &[u8] = match machine {
        // ldr x0, a; ldr x1, l; mov x8, #12; svc #0 (twice); mov x8, #3; svc #0; b .; nop; a; l
        Machine::Aarch64 => &[
            0x80, 0x01, 0x00, 0x58, 0xa1, 0x01, 0x00, 0x58, 0x88, 0x01, 0x80, 0xd2, 0x01, 0x00,
            0x00, 0xd4, 0x00, 0x01, 0x00, 0x58, 0x21, 0x01, 0x00, 0x58, 0x88, 0x01, 0x80, 0xd2,
            0x01, 0x00, 0x00, 0xd4, 0x68, 0x00, 0x80, 0xd2, 0x01, 0x00, 0x00, 0xd4, 0x00, 0x00,
            0x00, 0x14, 0x1f, 0x20, 0x03, 0xd5,
        ],
        // auipc t0,0; addi t0,t0,56; (ld a0,0(t0); ld a1,8(t0); li a7,12; ecall) x2;
        // li a7,3; ecall; j .; nop; a; l
        Machine::Riscv64 => &[
            0x97, 0x02, 0x00, 0x00, 0x93, 0x82, 0x82, 0x03, 0x03, 0xb5, 0x02, 0x00, 0x83, 0xb5,
            0x82, 0x00, 0x93, 0x08, 0xc0, 0x00, 0x73, 0x00, 0x00, 0x00, 0x03, 0xb5, 0x02, 0x00,
            0x83, 0xb5, 0x82, 0x00, 0x93, 0x08, 0xc0, 0x00, 0x73, 0x00, 0x00, 0x00, 0x93, 0x08,
            0x30, 0x00, 0x73, 0x00, 0x00, 0x00, 0x6f, 0x00, 0x00, 0x00, 0x13, 0x00, 0x00, 0x00,
        ],
        // (mov rdi,[rip+a]; mov rsi,[rip+l]; mov eax,12; int 0x80) x2; mov rdi,rax; mov eax,3;
        // int 0x80; jmp .; nop; a; l
        Machine::X86_64 => &[
            0x48, 0x8b, 0x3d, 0x31, 0x00, 0x00, 0x00, 0x48, 0x8b, 0x35, 0x32, 0x00, 0x00, 0x00,
            0xb8, 0x0c, 0x00, 0x00, 0x00, 0xcd, 0x80, 0x48, 0x8b, 0x3d, 0x1c, 0x00, 0x00, 0x00,
            0x48, 0x8b, 0x35, 0x1d, 0x00, 0x00, 0x00, 0xb8, 0x0c, 0x00, 0x00, 0x00, 0xcd, 0x80,
            0x48, 0x89, 0xc7, 0xb8, 0x03, 0x00, 0x00, 0x00, 0xcd, 0x80, 0xeb, 0xfe, 0x66, 0x90,
        ],
    };
...
/// A program that calls `SYS_WRITE_CONSOLE(its own first byte, len)` `count` times and exits with
/// 0 (ADR-204): the boot suite's measure that the write path allocates nothing per call. `count`
/// and `len` are two 64-bit words at the end of the code, loaded PC-relative.
pub fn loop_writer_code(machine: Machine, count: u64, len: u64) -> Vec<u8> {
    let template: &[u8] = match machine {
        // 0: ldr x9, n; 1: adr x0, 0b; ldr x1, l; mov x8, #12; svc #0; subs x9, x9, #1; b.ne 1b;
        // mov x0, #0; mov x8, #3; svc #0; b .; nop; n; l
        Machine::Aarch64 => &[
            0x89, 0x01, 0x00, 0x58, 0xe0, 0xff, 0xff, 0x10, 0x81, 0x01, 0x00, 0x58, 0x88, 0x01,
            0x80, 0xd2, 0x01, 0x00, 0x00, 0xd4, 0x29, 0x05, 0x00, 0xf1, 0x61, 0xff, 0xff, 0x54,
            0x00, 0x00, 0x80, 0xd2, 0x68, 0x00, 0x80, 0xd2, 0x01, 0x00, 0x00, 0xd4, 0x00, 0x00,
            0x00, 0x14, 0x1f, 0x20, 0x03, 0xd5,
        ],
        // 0: auipc t0,0; addi t0,t0,56; ld t1,0(t0); 1: auipc a0,0; addi a0,a0,-12; ld a1,8(t0);
        // li a7,12; ecall; addi t1,t1,-1; bnez t1,1b; li a0,0; li a7,3; ecall; j .; nop; n; l
        Machine::Riscv64 => &[
            0x97, 0x02, 0x00, 0x00, 0x93, 0x82, 0x82, 0x03, 0x03, 0xb3, 0x02, 0x00, 0x17, 0x05,
            0x00, 0x00, 0x13, 0x05, 0x45, 0xff, 0x83, 0xb5, 0x82, 0x00, 0x93, 0x08, 0xc0, 0x00,
            0x73, 0x00, 0x00, 0x00, 0x13, 0x03, 0xf3, 0xff, 0xe3, 0x14, 0x03, 0xfe, 0x13, 0x05,
            0x00, 0x00, 0x93, 0x08, 0x30, 0x00, 0x73, 0x00, 0x00, 0x00, 0x6f, 0x00, 0x00, 0x00,
        ],
        // 0: mov rbx,[rip+n]; 2: lea rdi,[rip+0b]; mov rsi,[rip+l]; mov eax,12; int 0x80; dec rbx;
        // jnz 2b; xor edi,edi; mov eax,3; int 0x80; jmp .; nop; n; l
        Machine::X86_64 => &[
            0x48, 0x8b, 0x1d, 0x29, 0x00, 0x00, 0x00, 0x48, 0x8d, 0x3d, 0xf2, 0xff, 0xff, 0xff,
            0x48, 0x8b, 0x35, 0x23, 0x00, 0x00, 0x00, 0xb8, 0x0c, 0x00, 0x00, 0x00, 0xcd, 0x80,
            0x48, 0xff, 0xcb, 0x75, 0xe6, 0x31, 0xff, 0xb8, 0x03, 0x00, 0x00, 0x00, 0xcd, 0x80,
            0xeb, 0xfe, 0x0f, 0x1f, 0x40, 0x00,
        ],
    };
    let mut code = template.to_vec();

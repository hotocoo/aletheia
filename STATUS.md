...
383, every expected map); the interactive boot's suite time fell 3847 -> 1410 ms on aarch64 (63%), 2265 -> 1150 ms on
riscv64 (49%), and on x86-64 to 436 ms on the input gate's machine (which has no remapping unit; the VT-d suite stays
before the prompt where one exists, because enforcement is a security property). Before it: BOOT COST, MEASURED — ADR-162. A stopwatch over the Hal clock
...
### 2026-09-27 — a program is handed its arguments (ADR-206)

- `run NAME ARGS` puts up to 256 argument bytes at the top of the program's stack page and enters
  it with their address and length in the C-ABI argument registers; the Rust `hello` greets them
  (`hello from user mode: to the world`, live on all three CPUs). `usermode` 45/45/53.

### 2026-09-27 — a program written in Rust (ADR-205)

- `userland/` builds `hello` from Rust for aarch64, riscv64 and x86-64 into the one read+execute
  page the judge accepts; the ELFs live in `userland/bin/`, `scripts/check-userland.sh` (a new CI
  job) rebuilds them byte-identically, and each target seeds its own as `hello`. Runs at boot and
  live on all three CPUs, printing its line and exiting 55. `usermode` 43/43/51.

### 2026-09-27 — a program writes to the console (ADR-204)
...
- `run` frames are preemptible on all three CPUs; the timer ends each slice and a program is
  abandoned at 64. A new medium also holds `spin` (branch to itself): `run spin` is abandoned and
  the console answers, live, then `hello` still exits 55. The timer goes back to the desktop's pump
  (aarch64 GIC ownership by GICD_CTLR, x86-64 IRQ0 entry and the quiesced PIT restored).
  `usermode` 38/38/46.

### 2026-09-26 — a program cannot stop the machine by faulting (ADR-202)

- Every exception a user-mode program raises now reaches the supervisor on all three CPUs (before:
  aarch64 and riscv64 exited on anything but a page fault; x86-64 double-faulted on `#DE`). A new
  medium also holds `trap` (undefined first instruction): `run trap` is TERMINATED and `run hello`
  still exits with 55, live under the desktop; sixteen faulting runs at boot hold no record and
  leak nothing. `usermode` 36/36/44. Still open: a program that never yields (ADR-203).
...
- x86-64 used the TSC as its clock; `qemu64` does not advertise invariant TSC and the TSC stopped
  while idle (15 s of RTC, 0.47 s of `uptime`). The HAL now takes the ACPI HPET unless CPUID says
  the TSC is invariant; `uptime` tracks the RTC (15.03 s of 15), asserted on all three CPUs by
  `console-e2e.sh`. `docs/BOOT-COST.md` regenerated.
...
- The desktop transfers and flushes only the rects it redrew (a cursor move: 2 KiB instead of the
  8 MB framebuffer at 1080p). A freeing kernel heap (`kheap.rs`) is built and proved on the host; not
  yet wired as the global allocator.
...
- Integer UI scale from the display height (1080p/1440p 2x, 2160p 4x): the desktop lays out at a
  logical size and the compositor sink writes each logical pixel as a scaled block; the taskbar
  spans the screen. Screenshots in `docs/evidence/desktop-*-scale2.png`.
...
- `aletheiad console system1-storm` + `scripts/system1-storm-e2e.sh`: 1,000 seeded hostile requests,
  four concurrent storms, and wire abuse at the sidecar. PASS: 0 unsafe lines, 0 System-1 errors,
  p50 27.6 ms / p99 56.9 ms sequential, every abuse probe refused with the right status.
- Found and fixed: a one-option choice crashed the backend (now never asked; the sidecar refuses it
  with 400 and turns any other backend exception into a logged 500), and neighbouring storm seeds
  were identical (now splitmix64).
...
- New gate `scripts/desktop-fuzz-e2e.sh`: seeded random keyboard/tablet storms through QMP on
  aarch64, riscv64 and x86-64 (chords, edges, aimed chrome clicks, paced drags); no panic, the
  console answers after every batch, and the heap must plateau (last third < 16 KiB).
- New aarch64 `heaptrace` feature + `scripts/heaptrace-symbolize.py`: per-allocation backtraces.
  They attributed ~227 KB per 3,000 events to resize drags (per-motion reallocation), window
  reopen, menu hover, per-navigation page grids and regrowing history; all fixed. Last-third growth
  after the fixes: 7.2 KB / 0 / 3.2 KB over 3,000 events.
...
- **Acting on ADR-162's evidence.** The load tests - the storms (`fsstorm`, `schedstorm`, `shellstorm`, `wmstorm`), the soak, the ML-risk stress, the bench and aarch64's post-suite perf pass - are two thirds of aarch64's boot and half of riscv64's. They prove bounds under volume on this machine's own heap, and they prove the same thing on every boot while the gate image proves the identical code on every push. The contract suites (capabilities, memory, TLS, HTTP, renderer, policy, IOMMU) cost tens of milliseconds each and say what THIS machine does. They stay.
- **One constant per `kmain`:** `STORMS_AT_BOOT = !cfg!(feature = "interactive")`. The seven blocks and `bench::run()` run when true. The gate image (no feature) keeps every one of them and every expected-map count; conformance is unchanged at 383. The interactive image prints `[boot] deferred in this interactive image (ADR-163): bench, soak, mlrisk-stress, wmstorm, schedstorm, fsstorm, shellstorm - the gate image proves them on every push`.
- **What stays before the prompt, deliberately:** the VT-d suite on x86-64 (3.5 s under TCG). It programs a real remapping unit and PROVES enforcement; a machine that offered a console before its IOMMU was proved to enforce would be offering the wrong thing first.
- **Measured** (`docs/BOOT-COST.md`, second table): interactive-image suite time 1410 ms on aarch64 (gate 3847, 63% saved), 1150 ms on riscv64 (gate 2265, 49%), 436 ms on x86-64 on the input gate's machine, which carries no remapping unit (like-for-like against the gate image minus `dmar`, about a second: roughly half).
- **Proof:** three boot gates + conformance (383) green and unchanged; every interactive gate green (console-e2e ×3, desktop-e2e-dt, keyboard-e2e, vinput-e2e, https-e2e, browser-e2e), each boot log carrying the deferred line. No `selftest storms` verb yet: the storms need what the console does not own, and a verb that ran a third of them would misname what it proves.
